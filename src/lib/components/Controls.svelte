...
<div class="controls-panel">
  ...
  <div class="tabs">
    {#each tabs as tab}
      <button class:active={$uiState.activeTab === tab.id} on:click={() => setActiveTab(tab.id)}>
        {tab.label}
      </button>
    {/each}
  </div>
  ...
  <div class="tab-content">
      ...
      {#if selectedStandee}
        ...
        <div class="control-group">
          ...
          <input
            ...
            on:input={(e) => updateStandee('height', +e.currentTarget.value)}
          />
          <div class="presets">
        ...
        <div class="control-group">
          ...
          <input
            ...
            on:input={(e) => updateInstances(+e.currentTarget.value)}
          />
        </div>
        ...
        <div class="control-group">
          ...
          <label
            >Image <input
              ...
              on:input={(e) => updateStandee('imageMargin', +e.currentTarget.value)}
            /></label
          >
          <label
            >Feet <input
              ...
              on:input={(e) => updateStandee('feetMargin', +e.currentTarget.value)}
            /></label
          >
        </div>
        ...
        <div class="control-group">
          <label for="ruler-toggle">
            <input
              ...
              on:change={(e) => updateStandee('rulerEnabled', e.currentTarget.checked)}
            />
            Show Height Ruler
          </label>
        </div>
        ...
        <div class="control-group">
          ...
          {#if selectedStandee.instances[0]?.token?.enabled}
            <div class="sub-controls" style="margin-top: 10px; padding-left: 20px;">
              ...
              <div class="grid-2">
                <select
                  ...
                  on:change={(e) => {
                    ...
                    if (newMode === 'alphabetic' && (!newStart || !isNaN(Number(newStart))))
                      newStart = 'A';
                    if (newMode === 'numeric' && (!newStart || isNaN(Number(newStart))))
                      newStart = '1';
                    updateTokenSettings(newMode as 'numeric' | 'alphabetic', newStart || '1');
                  }}
                >
                ...
                <input
                  ...
                  on:change={(e) =>
                    updateTokenSettings(
                      (selectedStandee.tokenSettings?.mode || 'alphabetic') as
                        | 'numeric'
                        | 'alphabetic',
              ...
              <div class="grid-2">
                <label style="font-size: 0.8em;" for="token-offset-x"
                  >X <input
                    id="token-offset-x"
                    type="number"
                    value={selectedStandee.instances[0].token.position?.x || 0}
                    on:input={(e) => updateTokenPosition('x', +e.currentTarget.value)}
                  /></label
                >
                <label style="font-size: 0.8em;" for="token-offset-y"
                  >Y <input
                    id="token-offset-y"
                    type="number"
                    value={selectedStandee.instances[0].token.position?.y || 0}
                    on:input={(e) => updateTokenPosition('y', +e.currentTarget.value)}
                  /></label
                >
              </div>
              ...
              <div class="grid-2">
                <div class="color-control">
                  <label for="token-bg-color" style="font-size: 0.7em;">BG</label>
                  <input
                    id="token-bg-color"
                    type="color"
                    value={selectedStandee.instances[0].token.bgColor}
                    on:input={(e) => updateTokenStyle('bgColor', e.currentTarget.value)}
                    title="Background"
                  />
                </div>
                <div class="color-control">
                  <label for="token-text-color" style="font-size: 0.7em;">Text</label>
                  <input
                    id="token-text-color"
                    type="color"
                    value={selectedStandee.instances[0].token.textColor}
                    on:input={(e) => updateTokenStyle('textColor', e.currentTarget.value)}
                    title="Text"
                  />
                </div>
              </div>
              ...
              <select
                ...
                on:change={(e) => updateTokenStyle('size', e.currentTarget.value)}
              >
    ...
    {:else if $uiState.activeTab === 'export'}
      ...
      <div class="control-group">
        <div class="grid-2">
          <button on:click={() => handleExport('print')} disabled={isGenerating}>
            {isGenerating ? 'Generating...' : 'Open'}
          </button>
          <button on:click={() => handleExport('download')} disabled={isGenerating}>
  ...
  <footer class="panel-footer">
    <a
      href="https://github.com/oscar-eriksson/standeepdf"
      target="_blank"
