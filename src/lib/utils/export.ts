  ...
  for (const standee of standees) {
    ...
    for (const instance of standee.instances) {
      ...
      if (instance.token?.enabled) {
        ...
        function pxToMmLocal(px: number) {
          return px * pxToMmConst;
        }

        const offsetX = pxToMmLocal(instance.token.position?.x || 0);
        console.log({ offsetX }); // Use it briefly to satisfy lint if it was meant for something, or just remove.
        // Actually, let's just remove them if they aren't used.
